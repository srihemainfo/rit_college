        ...
        <div class="col-md-12">
            @can('course_enroll_master_create')
                <div style="margin-bottom: 10px;" class="row">
                    <div class="col-lg-12">
                        <a class="btn btn-success" href="{{ route('frontend.course-enroll-masters.create') }}">
                            {{ trans('global.add') }} {{ trans('cruds.courseEnrollMaster.title_singular') }}
                        </a>
                        <button class="btn btn-warning" data-toggle="modal" data-target="#csvImportModal">
                            {{ trans('global.app_csvImport') }}
                        </button>
                        @include('csvImport.modal', ['model' => 'CourseEnrollMaster', 'route' => 'admin.course-enroll-masters.parseCsvImport'])
                    </div>
                </div>
            @endcan
            <div class="card">
                ...
                <div class="card-body">
                    <div class="table-responsive">
                        <table class=" table table-bordered table-striped table-hover datatable datatable-CourseEnrollMaster">
                            <thead>
                                <tr>
                                    <th>
                                    ...
                                    <tr data-entry-id="{{ $courseEnrollMaster->id }}">
                                        ...
                                        <td>
                                            @can('course_enroll_master_show')
                                                <a class="btn btn-xs btn-primary" href="{{ route('frontend.course-enroll-masters.show', $courseEnrollMaster->id) }}">
                                                    {{ trans('global.view') }}
                                                </a>
                                            @endcan

                                            @can('course_enroll_master_edit')
                                                <a class="btn btn-xs btn-info" href="{{ route('frontend.course-enroll-masters.edit', $courseEnrollMaster->id) }}">
                                                    {{ trans('global.edit') }}
                                                </a>
                                            @endcan

                                            @can('course_enroll_master_delete')
                                                <form action="{{ route('frontend.course-enroll-masters.destroy', $courseEnrollMaster->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('course_enroll_master_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('frontend.course-enroll-masters.massDestroy') }}",
...
@endcan
  ...
  let table = $('.datatable-CourseEnrollMaster:not(.ajaxTable)').DataTable({ buttons: dtButtons })
