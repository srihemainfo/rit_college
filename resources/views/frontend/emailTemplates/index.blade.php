        ...
        <div class="col-md-12">
            @can('email_template_create')
                <div style="margin-bottom: 10px;" class="row">
                    <div class="col-lg-12">
                        <a class="btn btn-success" href="{{ route('frontend.email-templates.create') }}">
                                    ...
                                    <tr data-entry-id="{{ $emailTemplate->id }}">
                                        ...
                                        <td>
                                            @can('email_template_show')
                                                <a class="btn btn-xs btn-primary" href="{{ route('frontend.email-templates.show', $emailTemplate->id) }}">
                                                    {{ trans('global.view') }}
                                                </a>
                                            @endcan

                                            @can('email_template_edit')
                                                <a class="btn btn-xs btn-info" href="{{ route('frontend.email-templates.edit', $emailTemplate->id) }}">
                                                    {{ trans('global.edit') }}
                                                </a>
                                            @endcan

                                            @can('email_template_delete')
                                                <form action="{{ route('frontend.email-templates.destroy', $emailTemplate->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('email_template_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('frontend.email-templates.massDestroy') }}",
