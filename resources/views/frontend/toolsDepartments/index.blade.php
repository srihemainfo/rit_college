        ...
        <div class="col-md-12">
            @can('tools_department_create')
                <div style="margin-bottom: 10px;" class="row">
                    <div class="col-lg-12">
                        <a class="btn btn-success" href="{{ route('frontend.tools-departments.create') }}">
                                    ...
                                    <tr data-entry-id="{{ $toolsDepartment->id }}">
                                        ...
                                        <td>
                                            @can('tools_department_show')
                                                <a class="btn btn-xs btn-primary" href="{{ route('frontend.tools-departments.show', $toolsDepartment->id) }}">
                                                    {{ trans('global.view') }}
                                                </a>
                                            @endcan

                                            @can('tools_department_edit')
                                                <a class="btn btn-xs btn-info" href="{{ route('frontend.tools-departments.edit', $toolsDepartment->id) }}">
                                                    {{ trans('global.edit') }}
                                                </a>
                                            @endcan

                                            @can('tools_department_delete')
                                                <form action="{{ route('frontend.tools-departments.destroy', $toolsDepartment->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('tools_department_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('frontend.tools-departments.massDestroy') }}",
