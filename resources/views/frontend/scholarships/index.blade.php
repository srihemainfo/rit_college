        ...
        <div class="col-md-12">
            @can('scholarship_create')
                <div style="margin-bottom: 10px;" class="row">
                    <div class="col-lg-12">
                        <a class="btn btn-success" href="{{ route('frontend.scholarships.create') }}">
                                    ...
                                    <tr data-entry-id="{{ $scholarship->id }}">
                                        ...
                                        <td>
                                            @can('scholarship_show')
                                                <a class="btn btn-xs btn-primary" href="{{ route('frontend.scholarships.show', $scholarship->id) }}">
                                                    {{ trans('global.view') }}
                                                </a>
                                            @endcan

                                            @can('scholarship_edit')
                                                <a class="btn btn-xs btn-info" href="{{ route('frontend.scholarships.edit', $scholarship->id) }}">
                                                    {{ trans('global.edit') }}
                                                </a>
                                            @endcan

                                            @can('scholarship_delete')
                                                <form action="{{ route('frontend.scholarships.destroy', $scholarship->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('scholarship_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('frontend.scholarships.massDestroy') }}",
