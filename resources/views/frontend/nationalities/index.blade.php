        ...
        <div class="col-md-12">
            @can('nationality_create')
                <div style="margin-bottom: 10px;" class="row">
                    <div class="col-lg-12">
                        <a class="btn btn-success" href="{{ route('frontend.nationalities.create') }}">
                                    ...
                                    <tr data-entry-id="{{ $nationality->id }}">
                                        ...
                                        <td>
                                            @can('nationality_show')
                                                <a class="btn btn-xs btn-primary" href="{{ route('frontend.nationalities.show', $nationality->id) }}">
                                                    {{ trans('global.view') }}
                                                </a>
                                            @endcan

                                            @can('nationality_edit')
                                                <a class="btn btn-xs btn-info" href="{{ route('frontend.nationalities.edit', $nationality->id) }}">
                                                    {{ trans('global.edit') }}
                                                </a>
                                            @endcan

                                            @can('nationality_delete')
                                                <form action="{{ route('frontend.nationalities.destroy', $nationality->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('nationality_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('frontend.nationalities.massDestroy') }}",
