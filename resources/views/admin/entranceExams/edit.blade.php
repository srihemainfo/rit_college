...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.entranceExam.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.entrance-exams.update", [$entranceExam->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('name') ? 'is-invalid' : '' }}" name="name_id" id="name_id">
                    @foreach($names as $id => $entry)
                        <option value="{{ $id }}" {{ (old('name_id') ? old('name_id') : $entranceExam->name->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
                @if($errors->has('name'))
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('exam_type') ? 'is-invalid' : '' }}" name="exam_type_id" id="exam_type_id">
                    @foreach($exam_types as $id => $entry)
                        <option value="{{ $id }}" {{ (old('exam_type_id') ? old('exam_type_id') : $entranceExam->exam_type->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control date {{ $errors->has('passing_year') ? 'is-invalid' : '' }}" type="text" name="passing_year" id="passing_year" value="{{ old('passing_year', $entranceExam->passing_year) }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('scored_mark') ? 'is-invalid' : '' }}" type="text" name="scored_mark" id="scored_mark" value="{{ old('scored_mark', $entranceExam->scored_mark) }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('total_mark') ? 'is-invalid' : '' }}" type="text" name="total_mark" id="total_mark" value="{{ old('total_mark', $entranceExam->total_mark) }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('rank') ? 'is-invalid' : '' }}" type="text" name="rank" id="rank" value="{{ old('rank', $entranceExam->rank) }}">
                @if($errors->has('rank'))
