    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.class-rooms.update", [$classRoom->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('name') ? 'is-invalid' : '' }}" type="text" name="name" id="name" value="{{ old('name', $classRoom->name) }}">
                @if($errors->has('name'))
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('block') ? 'is-invalid' : '' }}" name="block_id" id="block_id">
                    @foreach($blocks as $id => $entry)
                        <option value="{{ $id }}" {{ (old('block_id') ? old('block_id') : $classRoom->block->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
                @if($errors->has('block'))
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('type') ? 'is-invalid' : '' }}" type="text" name="type" id="type" value="{{ old('type', $classRoom->type) }}">
                @if($errors->has('type'))
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('room_no') ? 'is-invalid' : '' }}" type="number" name="room_no" id="room_no" value="{{ old('room_no', $classRoom->room_no) }}" step="1">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('no_of_seat') ? 'is-invalid' : '' }}" type="number" name="no_of_seat" id="no_of_seat" value="{{ old('no_of_seat', $classRoom->no_of_seat) }}" step="1">
