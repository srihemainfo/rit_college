...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.leaveStaffAllocation.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.leave-staff-allocations.update", [$leaveStaffAllocation->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('user') ? 'is-invalid' : '' }}" name="user_id" id="user_id" required>
                    @foreach($users as $id => $entry)
                        <option value="{{ $id }}" {{ (old('user_id') ? old('user_id') : $leaveStaffAllocation->user->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
                @if($errors->has('user'))
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('academic_year') ? 'is-invalid' : '' }}" name="academic_year_id" id="academic_year_id" required>
                    @foreach($academic_years as $id => $entry)
                        <option value="{{ $id }}" {{ (old('academic_year_id') ? old('academic_year_id') : $leaveStaffAllocation->academic_year->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('no_of_leave') ? 'is-invalid' : '' }}" type="number" name="no_of_leave" id="no_of_leave" value="{{ old('no_of_leave', $leaveStaffAllocation->no_of_leave) }}" step="1" required>
