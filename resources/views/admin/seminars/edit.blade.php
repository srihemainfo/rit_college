    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.seminars.update", [$seminar->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('user_name') ? 'is-invalid' : '' }}" name="user_name_id" id="user_name_id">
                    @foreach($user_names as $id => $entry)
                        <option value="{{ $id }}" {{ (old('user_name_id') ? old('user_name_id') : $seminar->user_name->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('topic') ? 'is-invalid' : '' }}" type="text" name="topic" id="topic" value="{{ old('topic', $seminar->topic) }}">
                @if($errors->has('topic'))
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('remark') ? 'is-invalid' : '' }}" type="text" name="remark" id="remark" value="{{ old('remark', $seminar->remark) }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control date {{ $errors->has('seminar_date') ? 'is-invalid' : '' }}" type="text" name="seminar_date" id="seminar_date" value="{{ old('seminar_date', $seminar->seminar_date) }}">
