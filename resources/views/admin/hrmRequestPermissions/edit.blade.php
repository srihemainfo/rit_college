...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.hrmRequestPermission.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.hrm-request-permissions.update", [$hrmRequestPermission->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('user') ? 'is-invalid' : '' }}" name="user_id" id="user_id" required>
                    @foreach($users as $id => $entry)
                        <option value="{{ $id }}" {{ (old('user_id') ? old('user_id') : $hrmRequestPermission->user->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
                @if($errors->has('user'))
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('no_of_hours') ? 'is-invalid' : '' }}" type="text" name="no_of_hours" id="no_of_hours" value="{{ old('no_of_hours', $hrmRequestPermission->no_of_hours) }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('from_date') ? 'is-invalid' : '' }}" type="text" name="from_date" id="from_date" value="{{ old('from_date', $hrmRequestPermission->from_date) }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('reason') ? 'is-invalid' : '' }}" type="text" name="reason" id="reason" value="{{ old('reason', $hrmRequestPermission->reason) }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('approved_by') ? 'is-invalid' : '' }}" type="text" name="approved_by" id="approved_by" value="{{ old('approved_by', $hrmRequestPermission->approved_by) }}">
